...
import { applyGenerationError, applyLoadedConfig, createInitialAppModel, dedupeRecentTopics, findRecentTopicMatch, } from './app_state.js';
import { createDefaultRecordingApi, } from './app_context.js';
...
export async function startApp(documentRef = document, fetchImpl = fetch, recordingApi = createDefaultRecordingApi()) {
    ...
    rerender = () => {
        render(documentRef, elements, state.model, state.isSettingsOpen, state.transientBannerMessage, state.recordedUrl, history.callbacks);
        history.renderModal();
    };
    ...
    try {
        ...
        state.model = applyGenerationError(state.model, getRequestErrorMessage(error, LOAD_ERROR_STATUS));
        state.model = {
            ...state.model,
            flow: 'home',
        };
