...
export function createRecordingFeature(ctx: AppContext): RecordingFeature {
  ...
  const handleStartRecording = async (): Promise<void> => {
    if (!recordingApi.isSupported()) {
      state.model = applyRecordingError(
        state.model,
    ...
    try {
      ...
      state.activeStream = stream as StreamLike;
      ...
      state.model = applyRecordingError(
        state.model,
        'Microphone access was unavailable. Please try again.',
      );
  ...
  const handleAnalyzeRecording = async (): Promise<void> => {
    ...
    try {
      ...
      try {
        ...
        state.model = applyHistorySaveError(
          state.model,
      ...
      if (controller.signal.aborted) {
        ...
        state.model = applyAnalysisError(
          state.model,
